    ...
    return (
        <div className={style.container}>
            ...
            <div className={style.container_container}>
                {user && feedBackPosts && filter === 'all' && feedBackPosts.length > 0 ? feedBackPosts.map( (post, i) => {
                ...
                }) : user && filter !== 'all' && props[filter].length > 0 ? props[filter].map( (post, i) => {
...
export const getServerSideProps = async () => {
    ...
    data.forEach((val) => {
        if (val.tags === 'UI') {
            ...
            ux.push(val);
        } else if (val.tags === 'Enhancements') {
            enhancements.push(val);
        } else if (val.tags === 'Bug') {
            bug.push(val);
        } else if (val.tags === 'Feature') {
            feature.push(val);
        }
    })
